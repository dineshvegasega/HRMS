...
import com.vegasega.hrms.databinding.ItemEmployeesPerformanceScoreBinding
...
import com.vegasega.hrms.screens.main.profiles.ProfilesVM.Model
...
class DataVM @Inject constructor(private val repository: Repository): ViewModel() {
    ...
    fun departmentsList(callBack: ItemLeaveRequest.() -> Unit) = viewModelScope.launch {
        repository.callApi(
...
//                        showSnackBar(response.body()!!.message)
    ...
    val departmentsAdapter = object : GenericAdapter<ItemEmployeesPerformanceScoreBinding, Data>() {
        override fun onCreateView(
            inflater: LayoutInflater,
            parent: ViewGroup,
            viewType: Int
        ) = ItemEmployeesPerformanceScoreBinding.inflate(inflater, parent, false)

        override fun onBindHolder(
            binding: ItemEmployeesPerformanceScoreBinding,
            model: Data,
            position: Int
        ) {
            binding.apply {
