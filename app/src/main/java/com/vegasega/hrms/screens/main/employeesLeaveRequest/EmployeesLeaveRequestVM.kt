...
class EmployeesLeaveRequestVM @Inject constructor(private val repository: Repository): ViewModel() {
    ...
    fun employeesLeaveRequestList(callBack: ItemAttendanceList.() -> Unit) = viewModelScope.launch {
        repository.callApi(
...
//                        showSnackBar(response.body()!!.message)
    ...
    val employeesLeaveRequestAdapter = object : GenericAdapter<ItemEmployeeLeaveRequestBinding, Attendance>() {
    ...
    fun employeesLeaveRequestPost(requestBody: RequestBody, callback: () -> Unit) = viewModelScope.launch {
        repository.callApi(
            callHandler = object : CallHandler<Response<BaseResponseDC<JsonElement>>> {
                override suspend fun sendRequest(apiInterface: ApiInterface) =
                    apiInterface.changeUpdate(requestBody)
